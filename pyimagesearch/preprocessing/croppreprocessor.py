...
class CropPreprocessor:
    def __init__(self, width, height, horiz = True, inter = cv2.INTER_AREA):
        #store the target image width, height, whether or not horizontal flips should be included, along with the interpolation method used when resizing
        self.width = width
        self.height = height
        self.horiz = horiz
        self.inter = inter
    ...
    def preprocess(self, image):
        ...
        #loop over the coordinates (tọa độ), extract each of the crops and resize each of them to a fixed size
